...
def create_controls_row():
    ...
    with ui.row().classes("w-full mt-4 items-center justify-center gap-4") as row:
        with ui.button("", icon="refresh", on_click=lambda: reset_board()).classes("rounded-full w-12 h-12") as reset_btn:
            ui.tooltip("Reset Board")
        with ui.button("", icon="autorenew", on_click=lambda: generate_new_board(phrases)).classes("rounded-full w-12 h-12") as new_board_btn:
        ...
        ui_seed_label = ui.label(f"Seed: {today_seed}").classes("text-sm text-center").style(
